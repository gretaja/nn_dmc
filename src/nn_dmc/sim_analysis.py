...
def parse_log_file(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            # Check if the line contains average energy info
            elif "Number of Walkers" in line:
                try:
                    parts = line.split(":")
                    value_part = parts[1].split()[0]  # Extract the numeric value
                    value = float(value_part)

                    num_walkers.append(value)
                    
                except (ValueError, IndexError) as e:
                    print(f"Warning: Skipping line due to error: {e}")

            # Check if the line contains local energy info
            elif "Average local energy" in line:
                try:
                    parts = line.split(":")
                    value_part = parts[1].split()[0]  # Extract the numeric value
                    value = float(value_part)
    ...
    return highest, lowest, average, num_walkers, local

def plot_log_stats(file_path,ymax=None):
    highest, lowest, average, num_walkers, local = parse_log_file(file_path)
