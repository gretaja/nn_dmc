...
if __name__ == '__main__': #if using multiprocessing on windows / mac, you need to encapsulate using this line
    ...
    myDMC = pv.DMC_Sim(sim_name=f'{system_name}_{sim_num}',
                            output_folder=f'/home/output/{system_name}/200w_5000ts_1dt',
                            weighting='discrete', #or 'continuous'. 'continuous' keeps the ensemble size constant.
                            num_walkers=200, #number of geometries exploring the potential surface
                            num_timesteps=5000, #how long the simulation will go. (num_timesteps * delta_t atomic units of time)
                            equil_steps=5000, #how long before we start collecting wave functions
                            chkpt_every=5000, #checkpoint the simulation every "chkpt_every" time steps
                            wfn_every=4999, #collect a wave function every "wfn_every" time steps
                            desc_wt_steps=250, #number of time steps you allow for descendant weighting per wave function
                            atoms=atom_list,
